...
.btn{appearance:none;border:1px solid var(--accent);background:linear-gradient(#1a73e8,#4285F4);color:#fff;font-weight:700;padding:10px 14px;border-radius:10px;cursor:pointer;box-shadow:0 6px 16px rgba(26,115,232,.25)}
...
.copy-btn{
  ...
  color: #000000;
  transition:background .18s ease,border .18s ease,transform .08s ease;
}
.copy-btn:hover{background:#ffffff;border-color:#5c5c5c}
.copy-btn:active{background:#ffffff;transform:translateY(1px)}
.copy-btn[disabled]{opacity:.55;cursor:not-allowed;transform:none}
.copy-btn .material-symbols-outlined{font-size:18px;line-height:1}
</style>
</head>
<body>
  <div class="shell">
    <header>
      ...
      <h1>COPS Time Zone Converter</h1>
    </header>
    ...
    <div class="panel">
      ...
      <div class="out">
        ...
        <div class="copy-actions">
          <!-- ChatGPT-style copy buttons -->
          <button id="copyDateBtn" class="copy-btn" disabled title="Click after you convert">
            <span class="material-symbols-outlined">content_copy</span> Copy date
          </button>
          <button id="copyTimeBtn" class="copy-btn" disabled title="Click after you convert">
            <span class="material-symbols-outlined">content_copy</span> Copy time
          </button>
        </div>
      </div>
    </div>
...
/* OUTPUT FORMAT: DD/M/YYYY (e.g., 01/2/2026) */
function formatPTParts(m){
  const inPT=m.clone().tz(TARGET_TZ);
  return {dateStr:inPT.format("DD/M/YYYY"), timeStr:inPT.format("h:mm A")};
}
...
(function init(){
  ...
  convertBtn.onclick=()=>{
    ...
    /* Enable copy buttons after first successful conversion */
    copyDateBtn.disabled=false; copyTimeBtn.disabled=false;
    copyDateBtn.title="Copy the date"; copyTimeBtn.title="Copy the time";
  };

  /* Copy helpers with "Copied!" feedback + fallback */
  function flashCopy(btn, ok=true){
    const prev = btn.innerHTML;
    btn.innerHTML = `<span class="material-symbols-outlined">check</span> ${ok ? "Copied!" : "Copy failed"}`;
    btn.disabled = true;
    setTimeout(()=>{ btn.innerHTML = prev; btn.disabled = false; }, 900);
  }

  copyDateBtn.onclick = async () => {
    if(!lastResultDateStr) return;
    try{
      await navigator.clipboard.writeText(lastResultDateStr);
      flashCopy(copyDateBtn, true);
    }catch(e){
      const ta = document.createElement('textarea');
      ta.value = lastResultDateStr; document.body.appendChild(ta);
      ta.select(); document.execCommand('copy');
      document.body.removeChild(ta);
      flashCopy(copyDateBtn, true);
    }
  };
  copyTimeBtn.onclick = async () => {
    if(!lastResultTimeStr) return;
    try{
      await navigator.clipboard.writeText(lastResultTimeStr);
      flashCopy(copyTimeBtn, true);
    }catch(e){
      const ta = document.createElement('textarea');
      ta.value = lastResultTimeStr; document.body.appendChild(ta);
      ta.select(); document.execCommand('copy');
      document.body.removeChild(ta);
      flashCopy(copyTimeBtn, true);
    }
